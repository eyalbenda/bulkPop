...
N2hap = list(as.bit(rep(1,length(elegansNames))))
N2hapMale = list(as.bit(rep(1,sum(elegansChrom!="X"))))
Hhap = list(as.bit(rep(0,length(elegansNames))))
Hhap[[1]][which(elegansChrom=="V"&elegansPos>2e7)] = T
HhapMale = list(as.bit(rep(0,sum(elegansChrom!="X"))))
HhapMale[[1]][which(elegansChrom=="V"&elegansPos>2e7)] = T
recombProbsHawaiiN2 = as.numeric(unlist(tapply(elegansMap,elegansChrom,recombProbCalc)))
f1temp = newIndividual(hap1 = N2hap, hap2 = HhapMale,sex = male)
...
devtools::use_data(N2xCB4856.genome)
